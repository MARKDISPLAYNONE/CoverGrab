...
/**
 * Get both analytics IDs (for passing to backend)
 */
export function getAnalyticsIds(): { anonymousUserId: string; sessionId: string } {
  return {
...
export async function hashVideoId(videoId: string): Promise<string> {
  try {
    ...
    return `v_${hashHex.substring(0, 16)}`;
  } catch {
 ...
 */
export function detectLinkType(hostname: string): LinkType {
